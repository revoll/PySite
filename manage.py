...
from app.models.movie import Poster, Still, MovieType
...
if os.path.exists(environment_file):
    ...
    for line in open(environment_file):
        if line[0] == '#':  # comments
            continue
        var = line.split('=')
        if len(var) == 2:  # in case of equations
            key = var[0].strip()
            val = var[1].split('#')[0].strip()  # in case of annotation
            os.environ[key] = val
else:
...
def make_shell_context():
    return dict(app=app, db=db, User=User, Follow=Follow, Role=Role,
                ...
                Poster=Poster, MovieType=MovieType, Still=Still)
